...
<%@ page
	import="java.util.*,com.device.common.impl.EventsListEvent,com.device.po.NodeEvents"
%>
...
<HTML>
	...
	<BODY class="tab">
					...
					<tr>
						...
						<td class="border_b">
							<table width="100%" border="0" cellspacing="0" cellpadding="0"
								...
								<tr>
									<td class="font_text_right" width="15%">
										用户
									</td>
									<td align="left" width='20%'>
										<div class="inpit2_bg">
											<input id="name" name="name" value="" type="text" size="16" />
										</div>
									</td>
								</tr>
								<tr>
			...
			<tr>
				<td>
					<div id="listTableDivId"
						...
						<p>
							<%
								UsersListResult result = (UsersListResult) request
										.getAttribute("result");
								...
								<%
									if (c != null && c.size() > 0) {
										...
										while (it.hasNext()) {
											i++;
											Users item = it.next();
											String css = (i % 2 == 0) ? "t1" : "t2";
								%>
								<tr id="tr<%=item.getName()%>" class="<%=css%>">
									<td>
										<a
											href="javascript:selectUser(this,'<%=item.getName()%>');void(0);"><%=item.getName()%></a>
									</td>
		...
		function selectUser(obj, uname) {
			$("#listTableDivId tr").each(function(index) {
				var c = (index % 2 > 0) ? 't1' : 't2';
				$(this).removeClass("t3").addClass(c);
			});
			$("#tr" + uname).removeClass("t1").removeClass("t2").addClass("t3");
			parent.frames.nodes_detailf.location.href = "<%=path%>/nodes/users_queryUserNodes.sip?id=" + uname;
		}
		</script>
	</BODY>
</HTML>