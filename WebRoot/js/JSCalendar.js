...
var _imgqx = new Image();
	...
	_cldTabFrm.border = 0;
	//_cldTabFrm.height = 140;
	_cldTabFrm.style.pixelWidth = 250;
	//_cldTabFrm.style.pixelHeight = 0;
...
function whenMouseOverDateItem(){
	...
	if(e.tagName == "TD"){
		...
		//alert(_lstFocus.style.backgroundColor);
...
function createDateBox(sBoxName, sDfltValue){
	...
	var _str = "<INPUT READONLY STYLE='border:1px solid gray;text-align:center;cursor:default;display:none' NAME='tt' VALUE='" + sDfltValue + "' onclick=fillBlank(tt)>";
	document.write(_str);
}
...
function selectQustion(name,inputDate,path,callback){ 

	var _regExp = /^((19)|(20))[0-9]{2}-([0-9]|(0[0-9])|(1[0-2]))-([0-9]|([0-2][0-9])|([3][0-1]))$/g;
	if(inputDate.match(_regExp) != null){
	var xURL= path + "js/date.jsp?inputName="+name+"&inputDate="+inputDate+"&path="+path;
	}else{
	var xURL= path + "js/date.jsp?inputName="+name+"&inputDate=&path="+path;
	}
        var showx = event.screenX - event.offsetX - 120;
  	...
  	//var retval = window.open(xURL,"日历","height=159, width=203,left="+showx+",top="+showy+",toolbar=no,menubar=no, scrollbars=no, resizable=no, location=no,status=no");
       ...
       {
    	   var windowProps="dialogWidth:240px; dialogHeight:211px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px;directories:no;scroll:no;Resizable=no;help:no;status=0";
       }
       else
       {
           var windowProps="dialogWidth:200px; dialogHeight:180px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px;directories:no;scroll:no;Resizable=no;help:no";
  	    }
