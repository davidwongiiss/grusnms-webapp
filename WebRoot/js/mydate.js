function CalendarMinute(name,fName)
{
	...
	this.toString = function()
	{
		...
		str += "<tr>"
			str += "<td>"
				str += "<input style=\"height:22px; width:44px;font-size:10px;background:url(../images/time/btn_bg.jpg) no-repeat; margin:0 3px;margin-left:15px;border:none\" type=\"button\" value=\"保存\" onclick=\"save();\" />"
				str += "<input style=\"height:22px; width:44px;font-size:10px;background:url(../images/time/btn_bg.jpg) no-repeat;border:none\" type=\"button\" value=\"取消\" onclick=\"cancle();\" />"
				str += "&nbsp;&nbsp;&nbsp;"
					str += "</td>"
		str += "</table>"
		return str;
...
function CalendarCalendar(name,fName)
{
	...
	this.toString = function()
	{
		...
		str += "<tr>";
		str += "<td class=\"red\">日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td class=\"red\">六</td>";
		str += "</tr>";
		str += "</table>";
		str += this.drawDate();
		str += "</div>";
